...
**`scheduling` + `notifications` + `patients` + `auth` implementados, com uma primeira versão da interface web sobre `/api/v1`.** Next.js (App Router) + TypeScript estrito + ESLint/Prettier + Vitest + Tailwind v4, estrutura modular alinhada a `docs/architecture.md`. Schema Drizzle (8 tabelas domínio + 4 tabelas Better Auth em histórico de migração separado), `scheduling-repository.ts` (ADR-0015, inclui `listSessions` — leitura simples, sem `SERIALIZABLE`), `notifications-repository.ts` (ADR-0016), `patients-repository.ts`, `professionals-auth-repository.ts` (ADR-0017) e `scheduling-service.ts` (orquestra scheduling+notifications numa única transação `SERIALIZABLE`) implementados e validados contra Postgres real — ver `src/db/README.md`, `src/db/repositories/README.md`, `src/modules/auth/README.md`, ADR-0015/0016/0017. Módulo `auth`: identidade via Better Auth (e-mail/senha, sessão de 60 dias), provisionamento sem convite por token (signup exige `professional` pré-existente não vinculado), `getSessionUser`/`requireSessionUser`/`requireRole` como única via de proteção de rota. Novos repositórios tenant-scoped só leitura: `professionals-repository.ts`, `rooms-repository.ts`, `clinics-repository.ts`.

Rotas montadas: `/api/auth/[...all]` (Better Auth) e `/api/v1/{patients,professionals,rooms,sessions}` — cascas finas (ADR-0001) sobre os repositórios/serviço acima, com `error-response.ts` mapeando erros de domínio para status HTTP. UI (grupo de rotas `(app)`, guardado por `getSessionUser` no layout; grupo `(public)` para `/login`): login, lista/cadastro de paciente, agenda (grade por sala/horário, timezone explícito via `modules/scheduling/day-range.ts`, criação/edição de sessão via painel lateral) — fluxo vertical completo do MVP. Leitura inicial das páginas é direta via repositório (Server Component), nunca via self-fetch a `/api/v1`; mutações client-side vão pela API. O módulo `jobs` ainda não foi implementado.

Comandos: `npm run dev|build|start|lint|typecheck|test|test:watch|test:integration|format|db:generate|db:migrate|db:seed:dev|auth:schema:generate|auth:db:generate|auth:db:migrate`. `test:integration` exige Postgres real provisionado manualmente, com as duas migrations aplicadas (domínio e Better Auth) e `DATABASE_URL`/`BETTER_AUTH_SECRET`/`BETTER_AUTH_URL` setadas — nunca sobe/derruba container sozinho (ver `src/db/repositories/README.md`). Ambiente de desenvolvimento local (Postgres em Docker + seed) documentado no `README.md`.
