...
**`scheduling` + `notifications` + `patients` implementados.** Next.js (App Router) + TypeScript estrito + ESLint/Prettier + Vitest, estrutura modular alinhada a `docs/architecture.md`. Schema Drizzle (8 tabelas), `scheduling-repository.ts` (ADR-0015), `notifications-repository.ts` (ADR-0016), `patients-repository.ts` e `scheduling-service.ts` (orquestra scheduling+notifications numa única transação `SERIALIZABLE`) implementados e validados contra Postgres real — ver `src/db/README.md`, `src/db/repositories/README.md`, ADR-0015/0016. Nenhuma rota de API, ou o módulo `auth`/`jobs` foram implementados ainda.
