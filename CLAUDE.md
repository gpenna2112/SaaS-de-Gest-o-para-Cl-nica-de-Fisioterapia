...
Rotas montadas: `/api/auth/[...all]` (Better Auth) e `/api/v1/{patients,professionals,rooms,sessions}` — cascas finas (ADR-0001) sobre os repositórios/serviço acima, com `error-response.ts` mapeando erros de domínio para status HTTP. UI (grupo de rotas `(app)`, guardado por `getSessionUser` no layout; grupo `(public)` para `/login`): login, lista/cadastro de paciente, agenda (grade por sala/horário, timezone explícito via `modules/scheduling/day-range.ts`) e criação de sessão (`/agenda/nova`) — fluxo vertical completo do MVP. Leitura inicial das páginas é direta via repositório (Server Component), nunca via self-fetch a `/api/v1`; mutações client-side vão pela API. O módulo `jobs` ainda não foi implementado.
