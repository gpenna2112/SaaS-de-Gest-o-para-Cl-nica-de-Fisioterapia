...
O repositório está em `src/db/repositories/patients-repository.ts`: `createPatient`, `getPatient`, `listPatients`, `updatePatient`, `deactivatePatient`, `reactivatePatient`. Nenhuma operação precisa de `SERIALIZABLE` (sem contagem entre linhas concorrentes) — só atomicidade simples entre a escrita em `patients` e o registro em `audit_log`. Rotas em `src/app/api/v1/patients` (`GET`/`POST`/`PATCH /[patientId]` — `PATCH` cobre edição de campos e o toggle `active`, delegando a `updatePatient`/`deactivatePatient`/`reactivatePatient` conforme o corpo da requisição). UI de edição/desativação em `/pacientes/[patientId]`.
